...
if _G.PhantomWyrmXMM2IsAlreadyRunning then
...
Tabs.Visuals:AddParagraph({
    ...
    Content = "ALL items are in fact visual and not real. You do not get to keep any of the items after rejoining. They are only for show."
})
...
Tabs.Visuals:AddButton({
    ...
    Description = "(If they don't spawn, reset character)",
    Callback = function()
        local DataBase = require(ReplicatedStorage.Database.Sync.Item)
        local PlayerData = require(ReplicatedStorage.Modules.ProfileData)
        local newOwned = {}
...
Tabs.Visuals:AddSection("Item Spawner")

Tabs.Visuals:AddInput("WeaponInput", {
    Title = "Weapon Name",
...
Tabs.Visuals:AddSection("Duplication Options")

Tabs.Visuals:AddInput("MultiInput", {
    ...
    Callback = function(Value)
        ...
        if multiplier and multiplier > 0 then
            ...
            Fluent:Notify({ Title = "Multiplier Set", Content = "Set to x" .. multiplier, Duration = 2 })
        end
    end
})

Tabs.Visuals:AddInput("SpecificItemInput", {
    Title = "Specific Item to Duplicate",
    Placeholder = "e.g. Christmas Knife",
    Numeric = false,
    Finished = true,
    Callback = function(Value)
        _G.DupeSpecificItem = Value
        Fluent:Notify({ Title = "Item Set", Content = "Target: " .. Value, Duration = 2 })
    end
})

Tabs.Visuals:AddButton({
    Title = "Duplicate Inventory (Visual)",
    Callback = function()
        local UIPath
        ...
        if UIPath:FindFirstChild("Weapons") then
            for _, item in pairs(UIPath.Weapons.Items.Container:GetChildren()) do
                if item:FindFirstChild("Container") then
                    for _, weapon in pairs(item.Container:GetChildren()) do
                        if weapon:IsA("Frame") and weapon:FindFirstChild("ItemName") then
                            ...
                            if (not specificItem or itemName == specificItem) and itemName ~= "Default Knife" and itemName ~= "Default Gun" then
                                local amountLabel = weapon.Container.Amount
                                local amountText = amountLabel.Text
                                if amountText == "" or amountText == "None" then
                                    amountLabel.Text = "x" .. tostring(multiplier)
                                else
                                    local num = tonumber(amountText:match("x(%d+)"))
                                    if num then
                                        amountLabel.Text = "x" .. tostring(num * multiplier)
                                    end
                                end
                            end
                        end
        ...
        pcall(function()
            local petsContainer = UIPath.Pets.Items.Container.Current.Container
            for _, pet in pairs(petsContainer:GetChildren()) do
                if pet:IsA("Frame") and pet:FindFirstChild("Container") then
                    local amountLabel = pet.Container.Amount
                    local amountText = amountLabel.Text
                    if amountText == "" or amountText == "None" then
                        amountLabel.Text = "x" .. tostring(multiplier)
                    else
        ...
        Fluent:Notify({
            Title = "Success",
            Content = "Inventory visually duplicated!",
            Duration = 3
        })
    end
})
...
local VisualsSection = Tabs.Visuals:AddSection("Crosshair Settings")
...
Tabs.Visuals:AddToggle("EnableCrosshairSize", {
    ...
    Callback = function(Value)
        ...
        if not Value and CrosshairObject then
            CrosshairObject.Size = UDim2.new(0, 25, 0, 25)
        end
    end
})
...
Tabs.Visuals:AddSlider("CrosshairSize", {
    ...
    Callback = function(Value)
        ...
        if CrosshairSizeEnabled and CrosshairObject then
...
local SpectateDropdown = Tabs.Visuals:AddDropdown("CrosshairTexture", {
    ...
    Values = {"Anime2", "Anime", "Star2", "Star", "Crosshair", "Crosshair2"},
    Multi = false,
    Default = "Default",
    Callback = function(Value)
        if not CrosshairObject then
        ...
        if Value then
            pulseConnection = RunService.RenderStepped:Connect(function()
                ...
                if crosshair and crosshair:IsA("ImageLabel") then
                    ...
                    crosshair.Size = UDim2.new(0, newSize, 0, newSize)
                end
            end)
        else
            ...
            local crosshair = player.PlayerGui:FindFirstChild("Crosshair", true)
            if crosshair and crosshair:IsA("ImageLabel") then
                crosshair.Size = UDim2.new(0, baseSize, 0, baseSize)
            end
        end
    end
})
...
FBM:SetFolder("PhantomWyrmXUniversal/FloatingButtons")
SaveManager:SetFolder("PhantomWyrmXUniversal/SaveConfig")
