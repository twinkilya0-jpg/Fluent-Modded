...
local Window = Fluent:CreateWindow({
    Title = "PhantomWyrm Hub X - Overhaul - Mobile",
    SubTitle = "v3.5.3 Made By Carryxkn2",
    TabWidth = 160,
...
local Tabs = {
    ...
    List = Window:AddTab({ Title = "List", Icon = "code" }),
}
...
function DFunctions.CrouchFunction()
    ...
    local crouchType = config.Type or "Normal"
    ...
    local debounce = debounceTimes[crouchType] or 0.1
    ...
    if crouchType == "Ground" then
        if (not isOnGround and yVelocity < -1 and hrp.Position.Y - 2 <= humanoid.RootPart.Position.Y + 0.5) or isOnGround then
            if (now - config.lastTick) > debounce then
                pressCrouch()
            end
        else
            if (now - config.lastReleaseTick) > debounce then
                releaseCrouch()
            end
        end

    elseif crouchType == "Air" then
    ...
    elseif crouchType == "Normal" then
        if isOnGround or (not isOnGround and yVelocity < -1 and hrp.Position.Y - 2 <= humanoid.RootPart.Position.Y + 0.5) then
            if (now - config.lastTick) > debounce then
                pressCrouch()
            end
        else
...
do
    ...
    getgenv().EasyTrimp = S.ET
    S.speed = S.ET.BaseSpeed

    RS.RenderStepped:Connect(function()
        local dt = tick() - S.last
        S.last = tick()
        ...
        if S.ET.Enabled then
            local inAir = hum.FloorMaterial == Enum.Material.Air
            
            if inAir then
                S.airTick = S.airTick + dt
                while S.airTick >= 0.04 do
                    S.airTick = S.airTick - 0.04
                    S.speed = math.min(S.ET.BaseSpeed + S.ET.ExtraSpeed, S.speed + 0.1)
                end
            else
                S.airTick = 0
                S.speed = math.max(S.ET.BaseSpeed, S.speed - (2.5 * dt))
            end

            if not S.bv or S.bv.Parent ~= hrp then
                if S.bv then S.bv:Destroy() end
                S.bv = Instance.new("BodyVelocity")
                S.bv.MaxForce = Vector3.new(4e5, 0, 4e5)
                S.bv.Parent = hrp
            end
            
            local look = Cam.CFrame.LookVector
            S.bv.Velocity = Vector3.new(look.X, 0, look.Z).Unit * S.speed
        else
            if S.bv then 
                S.bv:Destroy() 
                S.bv = nil 
    ...
    Tabs.Misc:AddInput("ET_Base", {
        Title = "Base Speed", 
        Default = tostring(S.ET.BaseSpeed), 
        Numeric = true, 
        Finished = true, 
        Callback = function(v) 
            S.ET.BaseSpeed = tonumber(v) or 50 
            S.speed = S.ET.BaseSpeed
        end
    })
    ...
    Tabs.Misc:AddInput("ET_Extra", {
        ...
        Default = tostring(S.ET.ExtraSpeed), 
        Numeric = true, 
        Finished = true, 
        Callback = function(v) 
            S.ET.ExtraSpeed = tonumber(v) or 100 
        end
    })
end
...
RunService.RenderStepped:Connect(function()
    ...
    if airborne and not inAir then
        speed = math.max(getgenv().EasyTrimp.BaseSpeed, speed - 10)
    ...
    if getgenv().EasyTrimp.Enabled then
        if inAir then
            ...
            while airTick >= 0.04 do
                ...
                speed = math.min(getgenv().EasyTrimp.BaseSpeed + extra, speed + 0.1)
            end
        else
            airTick, airSum = 0, 0
            speed = math.max(getgenv().EasyTrimp.BaseSpeed, speed - (2.5 * dt))
        end
...
do
    ...
    local _S = {
        ET = getgenv().EasyTrimp or {Enabled = false, BaseSpeed = 50, ExtraSpeed = 100},
        last = tick(), 
        airTick = 0, 
        speed = 50, 
        airborne = false,
    ...
    getgenv().EasyTrimp = _S.ET
    _S.speed = _S.ET.BaseSpeed
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if _S.ET.Enabled then
            if inAir then
                _S.airTick = _S.airTick + dt
                while _S.airTick >= 0.04 do
                    _S.airTick = _S.airTick - 0.04
                    _S.speed = math.min(_S.ET.BaseSpeed + _S.ET.ExtraSpeed, _S.speed + 0.1)
                end
            else
                _S.airTick = 0
                _S.speed = math.max(_S.ET.BaseSpeed, _S.speed - (2.5 * dt))
            end

            if not _S.bv or _S.bv.Parent ~= hrp then
                if _S.bv then _S.bv:Destroy() end
                _S.bv = Instance.new("BodyVelocity")
                _S.bv.MaxForce = Vector3.new(4e5, 0, 4e5)
                _S.bv.Parent = hrp
            end
            
            local moveDir = camera.CFrame.LookVector
            _S.bv.Velocity = Vector3.new(moveDir.X, 0, moveDir.Z).Unit * _S.speed
        else
            if _S.bv then _S.bv:Destroy() _S.bv = nil end
            _S.speed = _S.ET.BaseSpeed
        end
        _S.airborne = inAir
    end)
    ...
    Tabs.Misc:AddToggle("ET_BtnShow", {Title = "Show Trimp Button", Default = false}):OnChanged(function(s)
    if s then 
        local offset = DConfiguration.Settings.GuiScale.EasyTrimp
        DFunctions.CreateButton("ET_Btn", _S.ET.Enabled and "TRIMP: ON" or "TRIMP: OFF", 0.15 + offset, 0.1 + offset, function(btn) 
            _S.ET.Enabled = not _S.ET.Enabled 
            if btn and btn.Text then
                btn.Text = _S.ET.Enabled and "TRIMP: ON" or "TRIMP: OFF"
            end
        end)
    else 
        _S.ET.Enabled = false
    ...
    Tabs.Misc:AddInput("ET_ButtonSize", {
        ...
        Callback = function(Value)
            local num = tonumber(Value)
            if num then
                DConfiguration.Settings.GuiScale.EasyTrimp = num * 0.01
            else
                DConfiguration.Settings.GuiScale.EasyTrimp = 0
            end
    ...
    Toggle:OnChanged(function(State)       
       if State then
	         DFunctions.CreateButton("AutoCrouchGui", "Auto Crouch: OFF", 0.15 + DConfiguration.Settings.GuiScale.AutoCrouch, 0.1 + DConfiguration.Settings.GuiScale.AutoCrouch, function(btn)
	         	DConfiguration.Misc.MovementModification.BHOP.Crouch.FloatingButton = not DConfiguration.Misc.MovementModification.BHOP.Crouch.FloatingButton
	             btn.Text = DConfiguration.Misc.MovementModification.BHOP.Crouch.FloatingButton and "Auto Crouch: ON" or "Auto Crouch: OFF"
	             if not DConfiguration.Misc.MovementModification.BHOP.Crouch.FloatingButton then
	                LocalPlayer.PlayerScripts.Events.temporary_events.UseKeybind:Fire({ ["Key"] = "Crouch", ["Down"] = false })
	                wait(0.1)
	                LocalPlayer.PlayerScripts.Events.temporary_events.UseKeybind:Fire({ ["Key"] = "Crouch", ["Down"] = false })
	            end
...
local Dropdown = Tabs.Misc:AddDropdown("CrouchType", {
        ...
        Values = {"Ground", "Air", "Normal"},
        Multi = false,
        Default = 1,
    })

    Dropdown:OnChanged(function(Value)
        DConfiguration.Misc.MovementModification.BHOP.Crouch.Type = Value
    end)
...
RunService.Heartbeat:Connect(function()
    ...
    if DConfiguration.Misc.MovementModification.BHOP.Crouch.FloatingButton or DConfiguration.Misc.MovementModification.BHOP.Crouch.Keybind then
...
do
    ...
    local function createSpeedPadMonitor(pad)
        ...
        local connection = RunService.Heartbeat:Connect(function()
            ...
            local distance = (hrp.Position - padPosition).Magnitude 
            
            if distance <= 9 then
                if not speedPadObjects[padId].lastBoost or (tick() - speedPadObjects[padId].lastBoost) > 1 then
                    pcall(function()
                        firesignal(ReplicatedStorage.Events.Character.SpeedBoost.OnClientEvent, "SpeedPad", featureStates.SpeedPadValue, featureStates.SpeedPadDuration, Color3.new(0.49, 0.6, 1))
                    end)
    ...
    local function setupSpeedPadBooster()
        ...
        if not speedPadObjects.childAddedConn then
        ...
        Callback = function(state)
            ...
            if state then
                ...
                speedPadCharAddedConn = player.CharacterAdded:Connect(function()
                    task.wait(2)
                    if featureStates.SpeedPad then setupSpeedPadBooster() end
                end)
            else
                cleanupSpeedPadObjects()
                if speedPadCharAddedConn then speedPadCharAddedConn:Disconnect() end
            end
        end
    })
    ...
    Tabs.Premium:AddInput("SpeedValueInput", {
        ...
        Callback = function(text)
            featureStates.SpeedPadValue = tonumber(text) or 1.3
        end
    })
    ...
    Tabs.Premium:AddInput("SpeedDurationInput", {
        ...
        Numeric = true,
        Callback = function(text)
            featureStates.SpeedPadDuration = tonumber(text) or 2
        end
    })
end
...
do
    ...
    local function startJumpMonitor()
        if jumpConnection then jumpConnection:Disconnect() end
        local deployables = workspace:FindFirstChild("Game") and workspace.Game:FindFirstChild("Effects") and workspace.Game.Effects:FindFirstChild("Deployables")
        if not deployables then return end

        jumpConnection = RunService.Heartbeat:Connect(function()
            if not featureStates.JumpPadBooster then return end
            local hrp = getHRP()
            if not hrp then return end

            for _, pad in ipairs(deployables:GetChildren()) do
                if string.find(pad.Name:lower(), "jump") then
                    if (hrp.Position - pad:GetPivot().Position).Magnitude <= 7 then
                        hrp.Velocity = Vector3.new(hrp.Velocity.X, featureStates.JumpPadValue, hrp.Velocity.Z)
                    end
                end
            end
        end)
    ...
    Tabs.Premium:AddToggle("JumpPadToggle", {
        ...
        Callback = function(state)
            ...
            if state then startJumpMonitor() end
        end
    })
    ...
    Tabs.Premium:AddInput("JumpInput", {
        ...
        Callback = function(v) 
            ...
            if val then featureStates.JumpPadValue = val end
        end
    })
end
...
local TagSection = Tabs.Visual:AddSection("Nametag Changers")
...
Tabs.Info:AddParagraph({
    Title = "Script PhantomWyrm Hub X",
    Content = "By Carryxkn2",
})

Tabs.Info:AddParagraph({
    Title = "Legacy Script Version 2.9.1",
    Content = "By Carryxkn2",
})

Tabs.Info:AddParagraph({
    Title = "MM2 Script v2.3.5",
    Content = "By Carryxkn2",
})
...
local Dropdown = Tabs.List:AddDropdown("EmoteList", {
    Title = "EmoteList",
    Values = EmoteNames,
    Multi = false,
    Default = 1,
